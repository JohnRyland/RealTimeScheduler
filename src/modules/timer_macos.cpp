...
bool initialize(timer_t*, interrupt_controller_vtable_t&)
{
  return true;
}

bool set_speed(timer_t*, uint32_t /*speed*/)
{
  return true;
}

bool enable_timer(timer_t*)
{
  return true;
}

uint64_t current_tick(timer_t*)
{
  return current_tick_;
}

//uint64_t create_timer(timer_t*, timer_type_t type, uint64_t first_tick, uint64_t ticks_between, func_t callback, timer_t& timer)
uint64_t create_timer(timer_t*, timer_type_t , uint64_t , uint64_t , func_t , timer_t& )
{
  return 0ULL;
}

//uint64_t cancel_timer(timer_t* driver, timer_t& timer)
uint64_t cancel_timer(timer_t*, timer_t& )
{
  return 0ULL;
}

static
timer_vtable_t timer_macos_vtable =
{
...
module_t timer_macos_module =
{
  .type       = module_class::TIMER_DRIVER,
  .id         = 0x12023, // TODO: how to assign these? in the register?
  .name       = { "timer_macos" },
  .next       = nullptr,
  .prev       = nullptr,
  .vtable     = &timer_macos_vtable,
  .instance   = nullptr,
};

void register_timer_macos_module()
{
  module_register(timer_macos_module);
}

#endif // ENABLE_TIMER_MACOS
